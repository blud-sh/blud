...
export default function ChatroomPage() {
    ...
    const filteredChatrooms = chatrooms.filter((room) =>
        room.name.toLowerCase().includes(chatroomSearch.toLowerCase())
    )
        ...
        return (
            ...
            >
                <div className={`flex gap-3 ${isCurrentUser ? "flex-row-reverse" : ""}`}>
                    ...
                    {/* message content */}
                    <div
                        className={`flex flex-col max-w-xs sm:max-w-md ${isCurrentUser ? "items-end" : "items-start"}`}
                    >
                        {/* user name and time */}
                        {!isCurrentUser && (
                ...
                {hoveredMessage === message.id && (
                    <div
                        className={`absolute top-0 transform -translate-y-full z-20 ${
